...
void dgBody::UpdateCollisionMatrixSimd(dgFloat32 timestep, dgInt32 threadIndex)
{
  ...
  {
    dgFloat32 padding = m_collision->GetBoxMaxRadius()
        - m_collision->GetBoxMinRadius();
    dgFloat32 maxOmega = (m_omega % m_omega) * timestep * timestep;
    padding = (maxOmega > 1.0f) ? padding : padding * dgSqrt (maxOmega);
    dgVector step(
        m_veloc.Scale(timestep)
            + m_accel.Scale(m_invMass.m_w * timestep * timestep));
...
//			}
  ...
  if (m_collisionCell.m_cell)
  {
    ...
    if (!m_sleeping)
    {
      if ((dgAbsf(oldP0.m_x - m_minAABB.m_x) > DG_AABB_ERROR)
          || (dgAbsf(oldP0.m_y - m_minAABB.m_y) > DG_AABB_ERROR)
          || (dgAbsf(oldP0.m_z - m_minAABB.m_z) > DG_AABB_ERROR)
          || (dgAbsf(oldP1.m_x - m_maxAABB.m_x) > DG_AABB_ERROR)
          || (dgAbsf(oldP1.m_y - m_maxAABB.m_y) > DG_AABB_ERROR)
          || (dgAbsf(oldP1.m_z - m_maxAABB.m_z) > DG_AABB_ERROR))
      {
...
//			dgVector buoyanceCenter (volumeIntegral - m_matrix.m_posit);
      ...
      dgFloat32 damp = GetMax(
          GetMin(
              ...
              dgFloat32(dgFloat32(1.0f))), dgFloat32(dgFloat32(10.0f)));
      ...
      damp = GetMax(
          GetMin(
              ...
              dgFloat32(0.25f)), dgFloat32(2.0f));
...
void dgBody::IntegrateVelocity(dgFloat32 timestep)
{
  ...
  m_globalCentreOfMass += m_veloc.Scale(timestep);
  while (((m_omega % m_omega) * timestep * timestep)
      > (DG_MAX_ANGLE_STEP * DG_MAX_ANGLE_STEP))
  {
    m_omega = m_omega.Scale(dgFloat32(0.8f));
  }
  ...
  if (omegaMag2
      > ((dgFloat32(0.0125f) * dgDEG2RAD) * (dgFloat32(0.0125f) * dgDEG2RAD)))
  {
  ...
  m_matrix.m_posit = m_globalCentreOfMass
      - m_matrix.RotateVector(m_localCentreOfMass);
    ...
    dgFloat32 timestep) const
{
  ...
  if (m_world->m_solverMode)
  {
    ...
    {
      massAccel *= (dgFloat32(2.0f) * dgFloat32(LINEAR_SOLVER_SUB_STEPS)
          / dgFloat32(LINEAR_SOLVER_SUB_STEPS + 1));
    }
  }
  ...
  if (m_invMass.m_w > dgFloat32(0.0f))
  {
    if (!m_freeze)
    {
      ...
      for (dgBodyMasterListRow::dgListNode* node =
          m_masterNode->GetInfo().GetFirst(); node; node = node->GetNext())
      {
